...
function createInputTemplate(properties, canRemove) {
  ...
  forEach(properties, function(prop) {
    template += '<input  class="wksInput bpp-table-row-columns-' + columns + ' ' +
...
function getPlaceholderCount(strSource) {
  //统计字符串中包含{}或{xxXX}的个数
  var thisCount = 0;
  strSource.replace(/wksInput/g, function (m, i) {
    //m为找到的{xx}元素、i为索引
    thisCount++;
  });
  return thisCount;
}
Array.prototype.letfind = function(sel){
  var arr = [];
  if( sel.charAt(0) == '.' ){
...
module.exports = function(options) {
  ...
  var factory = {
    ...
    addElement: function(element, node, event, scopeNode) {
      ...
      if(getPlaceholderCount(container.innerHTML)>=4){
        return
      }else{
